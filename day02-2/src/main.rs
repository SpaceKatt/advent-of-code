...
fn process_program(opcodes: &mut Vec<u32>, verb: u32, noun: u32) -> Result<u32, Box<dyn Error>> {
...
fn main() {
    for noun in 0..99 {
        for verb in 0..99 {
            ...
            match process_program(&mut input, noun, verb) {
                Ok(number) => match number {
                    19690720 => {
                        println!("Final result :: {} :: {}", noun, verb);
                        break;
                    },
                    _ => continue,
                },
